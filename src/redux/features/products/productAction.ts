...
export const loadProducts = createAsyncThunk<
  ...
  { rejectValue: string }
>('product/loadProducts', async ({ page = 0, size = 9 }, { rejectWithValue }) => {
  try {
    const response = await api.get<ProductResponse>(`/products/all?page=${page}&size=${size}`)
...
export const findProduct = createAsyncThunk<Product, { id: number }, { rejectValue: string }>(
  'product/findProduct',
